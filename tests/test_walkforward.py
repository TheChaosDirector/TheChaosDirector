...
def test_folds_respect_embargo_and_chronology():
    ...
    folds = make_folds(n_dates=1500, cfg=cfg, warmup=200)
    assert len(folds) >= 2
    ...
    for f in folds:
        ...
        assert f.test_end <= 1499, "fold needs a next-day price for its last step"

    # Consecutive folds march forward in time.
